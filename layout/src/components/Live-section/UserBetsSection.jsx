...
const UserBetsSection = ({ userId, eventId }) => {
  ...
  return (
    <div className="bg-live-tertiary rounded border border-live-accent">
      <div className="border-b border-live-accent px-2 sm:px-3 py-1.5 sm:py-2">
        <h3 className="text-xs sm:text-sm font-bold text-live-accent">My Bets</h3>
      </div>
      <div className="p-1.5 sm:p-2" style={{ maxHeight: '160px', overflowY: 'auto' }}>
        {bets.map((bet, index) => (
          <div key={index} className="flex justify-between items-center py-1.5 sm:py-2 border-b border-live-hover last:border-b-0">
