...
export class UsersController {
  ...
  async create(
    ...
    try {
      ...
      const looksBcryptHashed = /^\$2[aby]\$\d{2}\$[./A-Za-z0-9]{53}$/.test(
        rawPassword,
      );
      console.log('Password hashing check:', {
        looksBcryptHashed,
        rawPassword,
      });
      payload.password = looksBcryptHashed
        ? rawPassword
  ...
  async login(
    ...
    try {
      ...
      const existingLoginHistory =
        await this.loginHistoryService.findByEmailAndIPs(
          user.email,
          loginDto.system_ip,
          loginDto.browser_ip,
        );
